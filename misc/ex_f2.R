...
other_population_string <- ""

# Parse the input strings into vectors
poplist <- parse_populations(input_string)
poplist_other <- parse_populations(other_population_string)

# Combine both lists and remove duplicates
combined_poplist <- unique(c(poplist, poplist_other))

# Create the combined population string
combined_poplist_string <- paste(combined_poplist, collapse = ",")

# Print the combined population string
cat("Combined Population List:", combined_poplist_string, "\n")

# Example function call with combined population list
# Note: Adjust the parameters as necessary for your use case
extract_f2(prefix_ho, f2_folder3, pops = combined_poplist_string, overwrite = TRUE, adjust_pseudohaploid = TRUE, qpfstats = FALSE, n_cores = 8, maxmem = 12000, auto_only = FALSE, maxmiss = 1)

# extract_f2_large(poplist, f2_folder, pops = combined_poplist, overwrite = TRUE, maxmiss = 1)
